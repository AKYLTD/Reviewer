  ...
  return (
    <div className="runscr" style={{ height: "100dvh", display: "flex", flexDirection: "column", gap: "clamp(6px, 1.2vh, 12px)", overflow: "hidden", padding: "clamp(8px,1.6vh,16px) clamp(10px,2.5vw,22px)" }}>
      ...
      <div style={{ position: "relative", flex: 1, minHeight: 0, background: C.card, borderRadius: 20, padding: "clamp(12px, 2.5vh, 26px) clamp(14px, 3vw, 30px)", textAlign: "center", border: `1px solid ${C.line}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {/* step text — big, bold, high-contrast, one sentence per line: readable from across the room */}
        <div className="display" style={{ fontSize: manyIng ? "clamp(20px, 3.8vw, 34px)" : "clamp(24px, 4.6vw, 46px)", fontWeight: 700, lineHeight: 1.24, color: C.ink, margin: "clamp(4px,1vh,10px) 0", flexShrink: 0, display: "grid", gap: "0.4em", justifyItems: "center" }}>
          {stepLines(step.text).map((ln, i) => <div key={i} style={{ maxWidth: 900 }}>{ln}</div>)}
        </div>
    ...
    <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, overflow: "hidden", marginBottom: 26 }}>
      ...
      {rows.length === 0 ? (
        ...
        <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          <div style={{ display: "grid", gridTemplateColumns: `1.6fr repeat(${cols.length}, 1fr) 0.9fr`, minWidth: 110 + cols.length * 110, gap: 10, padding: "12px 22px", fontSize: 13, fontWeight: 700, color: C.inkSoft, textTransform: "uppercase", letterSpacing: 0.5, borderBottom: `1px solid ${C.line}` }}>
            <span>Recipe</span>
            {cols.map((c) => <span key={c} style={{ textAlign: "right" }}>{c}</span>)}
            <span style={{ textAlign: "right" }}>Total</span>
          </div>
          {top.map((r, idx) => (
            <div key={r.recipe} style={{ display: "grid", gridTemplateColumns: `1.6fr repeat(${cols.length}, 1fr) 0.9fr`, minWidth: 110 + cols.length * 110, gap: 10, padding: "15px 22px", borderTop: idx ? `1px solid ${C.line}` : "none", fontSize: "clamp(16px,1.2vw,20px)", alignItems: "center" }}>
