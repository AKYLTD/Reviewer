...
/* Break a step's instruction into readable sentence lines so a long instruction never
   reads as one dense block. Splits on sentence ends and semicolons. */
...
/* Round to at most 2 decimals and drop trailing zeros — so 6.6000000000000005 → "6.6",
   40 → "40", 0.50 → "0.5". Kills floating-point noise in ingredient quantities. */
function fmtNum(n) {
  const v = Number(n);
...
/* Highlight any of `names` where they appear in `text`, in a distinct colour — used to
   tie a step's method text to the ingredient list. */
function HighlightText({ text, names }) {
  ...
  return <>{parts.map((p, i) => lower.has(p.toLowerCase())
    ? <span key={i} style={{ color: C.rustDeep, fontWeight: 800, background: C.gold, borderRadius: 5, padding: "0 4px" }}>{p}</span>
...
function App() {
  ...
  // Staff is written ONE ROW AT A TIME (never batched). The PIN column is write-only —
  // loaded staff have no pin in memory — so batching would mix rows with/without a pin
  // key, and PostgREST would reject that or blank the pin. Per-row upserts carry the pin
  // when the admin has set it and leave it untouched otherwise. Debounced per id so
  // typing coalesces into one save.
  const setStaffP = (updater) => setStaff((prev) => {
    const next = typeof updater === "function" ? updater(prev) : updater;
    if (readyRef.current) {
      const prevById = Object.fromEntries(prev.map((x) => [x.id, x]));
      const nextIds = new Set(next.map((x) => x.id));
      for (const s of next) {
        ...
        if (p && JSON.stringify(staffToRow(p)) === JSON.stringify(staffToRow(s))) continue; // unchanged
        const snapshot = { ...s }; // capture the row now (debounce fires later)
        scheduleSync("staff_" + s.id, () => upsertStaff(snapshot).catch((e) => {
          console.error("save staff failed", e);
          const msg = e.message || String(e);
          const dupPin = /pin/i.test(msg) && /unique|duplicate|staff_pin_key/i.test(msg);
          flash(dupPin ? `That PIN is already used by another person — pick a different one for ${snapshot.name || "this person"}.` : `⚠️ Couldn't save ${snapshot.name || "staff"} — ${msg}. Try again.`, 7000);
        }));
      }
      const removed = prev.filter((x) => !nextIds.has(x.id)).map((x) => x.id);
      if (removed.length && removed.length <= 3) removed.forEach((id) => deleteStaff(id).catch((e) => { console.error("delete staff failed", e); flash("Couldn't delete staff from the database"); }));
      else if (removed.length > 3) console.warn(`Skipped auto-delete of ${removed.length} staff rows (safety guard)`);
    }
    return next;
  });
  ...
  const resetReports = () => {
    setRuns([]); setCancellations([]); setAlertsP((al) => al.filter((a) => a.kind === "yield")); // keep unresolved yield warnings
    if (readyRef.current) dbResetReports().then(() => flash("Reports cleared")).catch((e) => { console.error("Reset reports failed", e); flash("Cleared on screen, but database write failed"); });
  };
  const deleteRuns = (ids) => {
    ...
    <div style={{ fontFamily: "'Nunito Sans',system-ui,sans-serif", background: C.cream, minHeight: "100vh", color: C.ink }}>
      ...
      <div style={{ maxWidth: screen === "view" ? "min(96vw,1600px)" : "min(96vw,2100px)", margin: "0 auto", padding: screen === "view" ? "18px clamp(16px,2.5vw,32px) 40px" : "30px clamp(16px,2.5vw,40px) 80px" }}>
        ...
        {screen === "yield" && finishing && !finishing._pickQty && (
          <YieldCheck production={finishing} onConfirm={confirmYield} onStop={() => cancelProduction(finishing)} />
        )}
  ...
  return (
    ...
    <div style={{ maxWidth, margin: "0 auto", padding: "15px clamp(16px,3vw,44px)", display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{ cursor: "pointer", display: "flex", alignItems: "baseline", gap: 9, minWidth: 0 }} onClick={onHome}>
        <span className="display" style={{ fontSize: "clamp(28px,2.1vw,44px)", fontWeight: 800, color: C.ink, whiteSpace: "nowrap" }}>Roni's<span style={{ color: C.rust }}>.</span></span>
...
function Home({ user, staff, recipes, ingredients, onSignIn, onPick, verifyPin, storeStock = {}, centralStock = {}, cpu, stores = [], runs = [], onOpenStock, onProduce }) {
    ...
    return (
      <div className="scr">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 16, marginTop: 26 }}>
          {staff.map((s) => (
            <button key={s.id} onClick={() => { setPinFor(s); setPin(""); setErr(false); }} style={{ background: C.card, border: pinFor?.id === s.id ? `2px solid ${C.rust}` : `1px solid ${C.line}`, borderRadius: 20, padding: "clamp(26px,2.4vw,40px) clamp(26px,2.4vw,40px) clamp(28px,2.6vw,44px)", cursor: "pointer", color: C.ink, textAlign: "left" }}>
              <div style={{ width: "clamp(56px,4vw,72px)", height: "clamp(56px,4vw,72px)", borderRadius: 999, background: s.role === "driver" ? C.rust : C.gold, color: "#fff", display: "grid", placeItems: "center", fontWeight: 800, fontSize: "clamp(25px,2vw,32px)", marginBottom: "clamp(18px,1.6vw,26px)" }}>{s.name[0]}</div>
              <div style={{ fontSize: "clamp(24px,2vw,36px)", fontWeight: 800 }}>{s.name}</div>
              <div style={{ fontSize: "clamp(16px,1.3vw,22px)", color: C.inkSoft, textTransform: "capitalize", marginTop: 4 }}>{s.role}</div>
            </button>
          ))}
        </div>
        ...
        <div style={{ marginTop: 30 }}>
          <StockPanel recipes={recipes} storeStock={storeStock} centralStock={centralStock} cpu={cpu} stores={stores} onOpenStock={onOpenStock} onProduce={onProduce} />
        </div>
        ...
        {pinFor && (
          <Modal onClose={() => { setPinFor(null); setPin(""); setErr(false); }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              {err && <p style={{ color: C.rust, fontSize: 14, marginBottom: 0, marginTop: 12 }}>{errKind === "server" ? "Couldn't reach the server — check the connection and try again." : "Wrong PIN — try again."}</p>}
            </div>
          </Modal>
        )}
      </div>
  ...
  return (
    <div className="scr">
      ...
      <h1 className="display" style={{ fontSize: "clamp(46px,5.4vw,92px)", fontWeight: 800, margin: "0 0 6px", lineHeight: 1.02 }}>What are we <span style={{ color: C.rust }}>making?</span></h1>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 4, alignItems: "stretch" }}>
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search recipes — type or tap the mic"
          style={{ flex: 1, background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: "18px 22px", fontSize: "clamp(19px,1.5vw,26px)", color: C.ink }} />
...
function Quantity({ recipe, ingredients, onBack, onStart }) {
  ...
  return (
    <div className="scr">
      ...
      <p style={{ fontSize: "clamp(19px,1.6vw,27px)", color: C.inkSoft, marginTop: 0 }}>How many {recipe.yieldUnit} do you need to produce?</p>
      ...
      <div style={{ background: C.card, borderRadius: 20, padding: 22, border: `1px solid ${C.line}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 8 }}>
          {items.map((it) => <div key={it.ingId} style={{ display: "flex", justifyContent: "space-between", padding: "11px 0", borderBottom: `1px solid ${C.line}` }}><span style={{ fontWeight: 400 }}>{m[it.ingId]?.name}</span><span className="display" style={{ fontWeight: 700 }}>{fmtNum(it.qty * factor)} {m[it.ingId]?.unit}</span></div>)}
        </div>
      </div>
...
function RunRecipe({ production, ingredients, recipes = [], onStep, onComplete, onCancel, onBack }) {
  ...
  const back = useCallback(() => {
    if (stepIndex > 0) onStep({ stepIndex: stepIndex - 1, durations: production.durations.slice(0, -1) });
  }, [stepIndex, onStep, production.durations]);
  ...
  const used = step.use || [];
  // full ingredient list across the whole recipe (scaled) — shown on every step so the
  // person always sees everything; the CURRENT step's items are highlighted.
  const aggAll = {};
  recipe.steps.forEach((st) => (st.use || []).forEach((u) => {
    const key = u.recipeId ? "r:" + u.recipeId : "i:" + u.ingId;
    if (!u.recipeId && !u.ingId) return;
    if (!aggAll[key]) aggAll[key] = { key, name: cName(u), unit: cUnit(u), qty: 0, isRecipe: !!u.recipeId };
    aggAll[key].qty += (Number(u.qty) || 0);
  }));
  const allItems = Object.values(aggAll);
  const curKeys = new Set(used.map((u) => u.recipeId ? "r:" + u.recipeId : "i:" + u.ingId));
  const curNames = used.map((u) => cName(u)).filter(Boolean);
  ...
  return (
    <div className="runscr" style={{ height: "100dvh", display: "flex", flexDirection: "column", gap: "clamp(6px, 1.2vh, 12px)", overflow: "hidden", padding: "clamp(8px,1.6vh,16px) clamp(10px,2.5vw,22px)" }}>
      ...
      <div style={{ position: "relative", flex: 1, minHeight: 0, background: C.card, borderRadius: 20, padding: "clamp(12px, 2.5vh, 26px) clamp(14px, 3vw, 30px)", textAlign: "center", border: `1px solid ${C.line}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {/* FULL ingredient list — always on top; this step's items highlighted in rust */}
        {allItems.length > 0 && (
          <div style={{ flexShrink: 0, display: "flex", flexWrap: "wrap", gap: "clamp(5px,1vw,9px)", justifyContent: "center", margin: "clamp(6px,1.3vh,12px) 0", maxHeight: "28vh", overflowY: "auto" }}>
            {allItems.map((it) => {
              const on = curKeys.has(it.key);
              return (
                <span key={it.key} className="display" style={{
                  display: "inline-flex", alignItems: "baseline", gap: 7,
                  background: on ? C.rust : C.cream, color: on ? "#fff" : C.inkSoft,
                  border: `1.5px solid ${on ? C.rust : C.line}`, borderRadius: 999,
                  padding: "clamp(5px,1vh,9px) clamp(11px,2vw,17px)",
                  fontSize: on ? "clamp(16px,2.8vw,24px)" : "clamp(13px,2vw,17px)", fontWeight: 700, opacity: on ? 1 : 0.7,
                }}>
                  {it.isRecipe ? "▸ " : ""}{it.name}
                  <b style={{ fontWeight: 800, whiteSpace: "nowrap" }}>{fmtNum(it.qty * factor)}{it.unit ? " " + it.unit : ""}</b>
                </span>
              );
            })}
          </div>
        ...
        {/* image */}
        {step.image && (
          <div style={{ flexShrink: 0, height: "clamp(56px, 12vh, 140px)", borderRadius: 14, margin: "clamp(4px,1vh,10px) auto", width: "min(340px, 100%)", background: `url(${step.image}) center/cover` }} />
        )}

        {/* method text — big, one sentence per line, this step's ingredients highlighted */}
        <div className="display" style={{ flex: 1, minHeight: 0, overflowY: "auto", display: "grid", gap: "0.4em", alignContent: "center", justifyItems: "center", fontSize: "clamp(22px, 4.2vw, 44px)", fontWeight: 700, lineHeight: 1.24, color: C.ink }}>
          {stepLines(step.text).map((ln, i) => <div key={i} style={{ maxWidth: 900 }}><HighlightText text={ln} names={curNames} /></div>)}
        </div>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "stretch", flexShrink: 0 }}>
        <button onClick={() => setShowStop(true)} style={{ background: "transparent", border: `1.5px solid ${C.rust}`, color: C.rust, borderRadius: 14, padding: "0 clamp(14px,3.5vw,24px)", fontWeight: 700, fontSize: "clamp(14px,2.4vw,17px)", cursor: "pointer", whiteSpace: "nowrap" }}>STOP</button>
        <button onClick={back} disabled={stepIndex === 0} title="Go back a step" style={{ background: "transparent", border: `1.5px solid ${C.inkSoft}`, color: C.inkSoft, borderRadius: 14, padding: "0 clamp(14px,3.5vw,24px)", fontWeight: 800, fontSize: "clamp(14px,2.4vw,17px)", cursor: stepIndex === 0 ? "not-allowed" : "pointer", opacity: stepIndex === 0 ? 0.4 : 1, whiteSpace: "nowrap" }}>‹ BACK</button>
...
function YieldCheck({ production, onConfirm, onStop }) {
  ...
  return (
    <div className="scr" style={{ maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 4 }}>
        {onStop && <button onClick={() => setShowStop(true)} style={{ background: "transparent", border: `1.5px solid ${C.rust}`, color: C.rust, borderRadius: 12, padding: "8px 18px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>STOP</button>}
      </div>
      ...
      {showStop && (
        <Modal onClose={() => setShowStop(false)}>
          <div className="display" style={{ fontSize: 26, fontWeight: 800, marginBottom: 8 }}>Stop this production?</div>
          <p style={{ color: C.inkSoft, marginTop: 0 }}>{recipe.name} · {targetQty} {unit} — this will be discarded and logged as cancelled.</p>
          <div style={{ display: "grid", gap: 12, marginTop: 12 }}>
            <BigButton full tone="rust" onClick={() => { setShowStop(false); onStop(); }}>DISCARD THIS PRODUCTION</BigButton>
            <button onClick={() => setShowStop(false)} style={{ background: "transparent", border: "none", color: C.inkSoft, fontWeight: 600, fontSize: 16, cursor: "pointer", padding: 10 }}>Keep going</button>
          </div>
        </Modal>
      )}
...
function Distribute({ production, stores, locations, delivery, onConfirm }) {
  ...
  return (
    <div className="scr">
      ...
      <p style={{ fontSize: "clamp(19px,1.6vw,27px)", color: C.inkSoft, marginTop: 0 }}>You made <b>{targetQty} {recipe.yieldUnit}</b> of {recipe.name}. Hold <b>+</b> to add all that's left · hold <b>−</b> to reset to 0.</p>
  ...
  if (!store) {
    return (
      <div className="scr">
        ...
        <p style={{ fontSize: "clamp(19px,1.6vw,27px)", color: C.inkSoft, marginTop: 0 }}>Pick a shop to see what's waiting for collection.</p>
...
function StockPanel({ recipes, storeStock, centralStock, cpu, stores, onOpenStock, onProduce }) {
    ...
    <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, overflow: "hidden", marginBottom: 26 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "14px 20px", borderBottom: `1px solid ${C.line}`, background: C.cardSoft, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span className="display" style={{ fontWeight: 800, fontSize: "clamp(20px,1.6vw,26px)" }}>Live stock</span>
        </div>
        ...
        <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          ...
          {top.map((r, idx) => (
            <div key={r.recipe} style={{ display: "grid", gridTemplateColumns: `1.6fr repeat(${cols.length}, 1fr) 0.9fr${onProduce ? " 104px" : ""}`, minWidth: 110 + cols.length * 110 + (onProduce ? 104 : 0), gap: 10, padding: "15px 22px", borderTop: idx ? `1px solid ${C.line}` : "none", fontSize: "clamp(16px,1.2vw,20px)", alignItems: "center" }}>
              ...
              {onProduce && <button onClick={() => onProduce(recipeByName[r.recipe])} style={{ background: C.go, color: "#fff", border: "none", borderRadius: 999, padding: "7px 0", fontWeight: 800, fontSize: 13, cursor: "pointer" }}>Produce</button>}
            </div>
          ))}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}><tbody>
              {items.map((it) => (
                <tr key={it.key} style={{ borderBottom: `1px solid ${C.line}` }}>
                  ...
                  <td className="display" style={{ padding: "11px 0", textAlign: "right", fontWeight: 800, whiteSpace: "nowrap", fontSize: "clamp(16px, 2.6vw, 20px)" }}>{fmtNum(it.qty)} {it.unit}</td>
                </tr>
              ))}
            </tbody></table>
          </div>
          ...
          <div style={{ display: "grid", gap: 22 }}>
            {recipe.steps.map((s, k) => (
              <div key={k} style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                <span className="display" style={{ fontSize: "clamp(22px, 3vw, 30px)", fontWeight: 800, color: C.gold, lineHeight: 1.2, minWidth: 30, flexShrink: 0 }}>{k + 1}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: "clamp(17px, 2.8vw, 23px)", lineHeight: 1.6, color: C.ink, fontWeight: 500, display: "grid", gap: "0.45em" }}>
                    {stepLines(s.text).map((ln, i) => <div key={i}>{ln}</div>)}
                  </div>
                  {s.image && (
            ...
            <tbody>
              {items.map((it) => (
                <tr key={it.key} style={{ borderBottom: `1px solid ${C.line}` }}>
                  ...
                  <td className="display" style={{ padding: "9px 0", textAlign: "right", fontWeight: 700, whiteSpace: "nowrap" }}>{fmtNum(it.qty)} {it.unit}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            {recipe.steps.map((s, k) => (
              <div key={k} style={{ display: "grid", gridTemplateColumns: "34px 1fr auto", gap: 12, alignItems: "start" }}>
                ...
                <div>
                  ...
                  {s.use && s.use.length > 0 && <div style={{ fontSize: 13, color: C.inkSoft, marginTop: 4 }}>{s.use.map((u) => `${fmtNum(u.qty)}${cUnit(u)} ${cName(u)}`).join(" · ")}</div>}
                </div>
  ...
  return (
    <div className="scr">
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
        ...
        <button onClick={() => { if (onAutoSave) onAutoSave(buildClean()); onCancel(); }} style={pillGhost}>{onAutoSave ? "Done" : "Cancel"}</button>
...
function AdminReports({ runs: allRuns, recipes, cancellations, storeStock, centralStock, deliveryQueue, stores, onResetStock, onResetReports, onDeleteRuns }) {
  ...
  const [range, setRange] = useState("all"); // all | today | 7 | 30
  const [sel, setSel] = useState(() => new Set()); // selected run ids for deletion
  ...
  return (
    <div>
      ...
      {view === "runs" && (
        <>
        {(onResetReports || onDeleteRuns) && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#F6E0D6", border: `1px solid ${C.rust}`, borderRadius: 14, padding: "10px 14px", marginBottom: 14, flexWrap: "wrap" }}>
            <span style={{ flex: 1, fontSize: 13, minWidth: 180 }}>Deleting runs removes them from reports and the leaderboard. Stock is <b>not</b> affected. This is permanent.</span>
            {onDeleteRuns && sel.size > 0 && (
              <button onClick={() => { if (window.confirm(`⚠️ Delete ${sel.size} selected run${sel.size > 1 ? "s" : ""} FOREVER? This cannot be undone.`)) { onDeleteRuns([...sel]); setSel(new Set()); } }} style={{ ...adminBtn, background: C.rust, color: "#fff", border: "none" }}>Delete selected ({sel.size})</button>
            )}
            {onResetReports && (
              <button onClick={() => { if (window.confirm("⚠️ Clear ALL reports (every production run and cancellation) FOREVER? Stock is kept. This cannot be undone.")) { onResetReports(); setSel(new Set()); } }} style={{ ...adminBtn, background: C.ink, color: "#fff", border: "none" }}>Reset all reports</button>
            )}
          </div>
        )}
        <div style={{ background: C.card, borderRadius: 16, overflowX: "auto", border: `1px solid ${C.line}` }}>
          <div style={{ display: "grid", gridTemplateColumns: "34px 1.4fr 0.7fr 0.8fr 0.7fr 0.7fr 0.7fr 0.7fr 0.8fr", minWidth: 714, gap: 10, padding: "12px 18px", borderBottom: `2px solid ${C.line}`, fontSize: 11, fontWeight: 700, color: C.inkSoft, textTransform: "uppercase", letterSpacing: 0.5, alignItems: "center" }}>
            <input type="checkbox" title="Select all" checked={runs.length > 0 && runs.every((r) => sel.has(r.id))} onChange={(e) => setSel(e.target.checked ? new Set(runs.map((r) => r.id)) : new Set())} style={{ width: 18, height: 18, accentColor: C.rust }} />
          ...
          {runs.length === 0 ? <div style={{ padding: 18, color: C.inkSoft }}>No runs yet.</div> : runs.map((r, idx) => {
            ...
            return (
              <div key={r.id} style={{ display: "grid", gridTemplateColumns: "34px 1.4fr 0.7fr 0.8fr 0.7fr 0.7fr 0.7fr 0.7fr 0.8fr", minWidth: 714, gap: 10, padding: "12px 18px", borderTop: idx ? `1px solid ${C.line}` : "none", fontSize: 14, alignItems: "center", background: sel.has(r.id) ? "#F6E0D6" : "transparent" }}>
                <input type="checkbox" checked={sel.has(r.id)} onChange={() => toggleSel(r.id)} style={{ width: 18, height: 18, accentColor: C.rust }} />
                <div><b>{r.recipe}</b><div style={{ fontSize: 11, color: C.inkSoft }}>{r.when}</div></div>
                <span>{fmtNum(r.qty)}{r.unit}</span>
                <span>{r.by}</span>
